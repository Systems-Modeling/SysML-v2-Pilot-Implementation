 ...
 *
 */
public class BandedRelationshipVisitor extends ElementVisitor {
	 ...
	 */
	private final NestedFeatureTraversal traversal;
	 ...
	 */
	public BandedRelationshipVisitor(Relationship element, NestedFeatureTraversal traversal, Element visitingFrom) {
	...
	protected void postProcess() {
		// if it moves through Containers, then navigate but don't extend
		if (isContainerRelationship(element)) {
			this.getTraversal().visit(lookAhead);
		}
		// if the relationship is the correct metatype, navigate forward with breadcrumbs to extend the paths
		else if (isPathRelationship(element)) {
	...
	boolean isPathRelationship(Relationship relationToCheck) {
		if (!(relationToCheck instanceof ConjugatedPortTyping) &&
				(relationToCheck instanceof FeatureTyping || relationToCheck instanceof FeatureMembership)) {
			return true;
		}
		return false;
	}
	
	boolean isContainerRelationship(Relationship relationToCheck) {
		// allow for drill-in to model on memberships (accept packages)
			if (relationToCheck instanceof Membership) {
				Element memberTest = ((Membership) relationToCheck).getMemberElement();
				Element ownerTest =((Membership) relationToCheck).getOwningRelatedElement();
				if (memberTest instanceof Package) {
					return true;
				}
				else if (memberTest instanceof Definition || memberTest instanceof Usage) {
					//traversal.pushToStack(((Membership) relationToCheck).getMemberElement());
				}
				if (ownerTest instanceof Package) {
					//System.out.println("Navigating from Package to " + memberTest.getEffectiveName());
					if (memberTest instanceof Definition || memberTest instanceof Usage) {
						((NestedFeatureTraversal) this.getTraversal()).startNewPath(memberTest);
					}
					return true;
				}
			}
