...
/**
 * Generated setting delegate for {@code TransitionUsage.effectAction}.
 */
public class TransitionUsage_effectAction_SettingDelegate extends AbstractSettingDelegate {

    public TransitionUsage_effectAction_SettingDelegate(EStructuralFeature feature) {
        super(feature);
    }

    @Override
    protected String delegateId() {
        return "TransitionUsage.effectAction";
    }
    ...
    protected Object get(InternalEObject owner, boolean resolve, boolean coreType) {
        /*
         * OCL2.0 deriveTransitionUsageEffectAction
         ...
         */
        var result = new BasicEList<ActionUsage>();
        if (owner instanceof TransitionUsage self) {
            for (var ownedFeatureMembership : self.getOwnedFeatureMembership()) {
                if (ownedFeatureMembership instanceof TransitionFeatureMembership transitionFeatureMembership
                        && transitionFeatureMembership.getKind() == TransitionFeatureKind.EFFECT
                        && transitionFeatureMembership.getTransitionFeature() instanceof ActionUsage actionUsage) {
                    result.add(actionUsage);
                }
            }
        }
        return result;
