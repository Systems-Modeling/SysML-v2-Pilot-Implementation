...
public class StepImpl extends FeatureImpl implements Step {
	...
	public EList<Subsetting> getOwnedSubsetting() {
		...
		return getOwnedSubsettingWithComputedRedefinitions(isSubperformance() ? STEP_SUBSETTING_PERFORMANCE_DEFAULT
				: isEnactedPerformance() ? STEP_SUBSETTING_OBJECT_DEFAULT
						: isIncomingTransfer() ? STEP_SUBSETTING_TRANSFER_DEFAULT : STEP_SUBSETTING_BASE_DEFAULT);
	}
	...
	public List<? extends Feature> getRelevantFeatures() {
		return getOwnedFeature().stream().filter(f -> f instanceof ItemFeature).collect(Collectors.toList());
	}
	...
	public boolean isEnactedPerformance() {
		...
		return owningType instanceof Class
				|| owningType instanceof Feature && ((FeatureImpl) owningType).isObjectFeature();
	}
	...
	public List<Step> getSubsteps() {
		return getOwnedFeature().stream().filter(f -> f instanceof Step).map(f -> (Step) f)
