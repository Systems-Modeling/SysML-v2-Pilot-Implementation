...
"""
bnf_grammar_processor is a command line tool that
1. can extract the textual and graphical BNF grammars from
   exports of the KerML and SysML specifications in raw HTML format
2. can parse and validate (manually) corrected BNF grammar files.

Detailed usage documentation is below in the main() function,
and also be obtained by running the tool with the --help option.
...
class Info(GrammarElement):
    ...
    def get_txt(self) -> str:
        rendered_lines = [
            ...
            line_comment(f"Generated by bnf_grammar_processor at: {self.timestamp}")
            ] + [line_comment(x) for x in self.lines]
        rendered_string = "".join(rendered_lines)
        return f"{rendered_string}\n\n"

    def get_html(self) -> str:
        rendered_lines = [
            f"{LINE_COMMENT_MARKER} Source document: {self.source}",
            f"{LINE_COMMENT_MARKER} Generated by bnf_grammar_processor at: {self.timestamp}"
...
class Production(GrammarElement):
    ...
    def get_html(self) -> str:
        # Resolve non-terminal references to create HTML hyperlinks
        html_lines: list[str] = []
        # is_textual = self.processor.syntax_kind == "textual-bnf"
        is_graphical = self.processor.syntax_kind == "graphical-bnf"
        for html_line in self.lines:
            tokens = GrammarProcessor.tokenize(html_line)
            # TODO: Refactor term resolution using the Lark grammar parsing instead of the tokenize function
            pieces = []
            for token in tokens:
                ...
                if token.kind == TokenKind.NON_TERMINAL:
                    if token.text == self.name:
                        ...
                        pass
                    elif is_pascal_case(token.text) or is_upper_snake_case(token.text) or (is_graphical and is_lower_kebab_case(token.text)):
                        ...
                        # Do not link to symbol or terminal or property
                        pass
        ...
        # Construct HTML hyperlinked markup
        rendered_lines = "\n".join(html_lines)
        if self.is_partial:
            production_anchor = ""
        else:
        ...
        hyperlinked_production = f"{production_anchor}<pre>{rendered_lines}\n</pre>\n"
...
class NoteList(GrammarElement):
    ...
    def get_marked_up_txt(self) -> str:
        list_tag_pattern = re.compile(r"(<(ol|ul|li)>)")
        snippet_with_linebreaks = list_tag_pattern.sub(r"\n\1", self.html_snippet).strip()
        rendered_lines = ["Notes:"]
        rendered_lines += snippet_with_linebreaks.split("\n")
        comment_prefix = f"{LINE_COMMENT_MARKER} "
        rendered_string = comment_prefix + f"\n{comment_prefix}".join(rendered_lines) + "\n\n"
        return rendered_string

    def get_txt(self) -> str:
        rendered_lines = ["Notes:"]
        rendered_lines += render_nested_lists(self.html_snippet, mode=RenderMode.TXT, apply_line_comment=False).split("\n")
        rendered_string = block_comment(rendered_lines)
        rendered_string = remove_html_tags(rendered_string)
        return rendered_string

    def get_html(self) -> str:
        rendered_lines = [f"<p>{LINE_COMMENT_MARKER} <strong>Notes:</strong></p>"]
...
class GrammarProcessor:
    ...
    def extract_bnf_from_spec(self, input_dir: str, output_dir: str, input_file: str, syntax_kind: str, bnf_clause_id: str):
        ...
        for tag in self.soup.find_all(name=["h1", "pre", "p", "ol"], recursive=True):
            ...
            # elif tag.name == "pre" and inside_bnf_clause and tag.parent.name != "li":
                ...
                # Store cleaned version of HTML note list
                note_list.html_snippet = remove_ws_surrounding_list_elements(normalize_whitespace(clean_text_content(str(tag))))
    ...
    def parse_marked_up_bnf(self, input_dir: str, output_dir: str, input_file: str, syntax_kind: str, bnf_clause_id: str):
        ...
        LOGGER.info(20 * "=")
        LOGGER.info(f"Processing BNF from {self.input_path} syntax-kind={syntax_kind} clause={self.bnf_clause_id} at {self.start_timestamp}")
        LOGGER.info(20 * "=")
        ...
        for candidate_production in candidate_productions:
            ...
            if candidate_production.startswith(LINE_COMMENT_MARKER):
                # process comment
                comment_clause_pattern = re.compile(r"^(// )(Clause|Part) ([0-9.]+) (.+)")
                clause_match = comment_clause_pattern.match(candidate_production)
                if clause_match:
                    if clause_match.group(2) == "Clause":
                        clause_id = clause_match.group(3)
                    else:
                        clause_id = ""
                    clause_title = clause_match.group(4)
                    LOGGER.info(f"Processing clause id={clause_id} title={clause_title}")
                    self.elements.append(Heading(clause_id=clause_id, lines=[candidate_production[3:]]))
                elif candidate_production.startswith("// Notes:"):
                    html_snippet_lines = candidate_production.split("\n")
                    html_snippet_lines.pop(0)
                    html_snippet = "".join([x[3:] for x in html_snippet_lines])
                    html_snippet = html_snippet.replace("\n", "")
                    self.elements.append(NoteList(clause_id=clause_id, lines=[], html_snippet=html_snippet))
                else:
                    self.elements.append(Comment(clause_id=clause_id, lines=candidate_production.split("\n")))
            else:
                # This should be a proper BNF production - check with Lark parser
                is_parse_successful = True
                production_name = ""
                abstract_syntax_type = ""
                try:
                    parse_tree = self.parser.parse(candidate_production + "\n")
                    first_token = parse_tree.children[0].children[0]
                    if first_token.type in ("NONTERMINAL_NAME", "LEXICAL_NAME", "GRAPHICAL_NAME"):
                        production_name = first_token.value
                    for token in parse_tree.find_data("abstract_element"):
                        if token.children[0].type == "NONTERMINAL_NAME":
                            abstract_syntax_type = token.children[0].value
                    if abstract_syntax_type == "" and first_token.type == "NONTERMINAL_NAME":
                        abstract_syntax_type = production_name

                    if self.syntax_kind == "graphical-bnf" and candidate_production.find("=|") >= 0:
                        is_partial = True
                        # LOGGER.info(f"PARTIAL PROD: {parse_tree.pretty(indent_str='  ')}")
                        if not production_name in self.partial_productions:
                            self.partial_productions[production_name] = []
                        # partial_terms = []
                        for count, token in enumerate(parse_tree.find_token("GRAPHICAL_NAME")):
                            if count > 0:
                                # partial_terms.append(token.value)
                                self.partial_productions[production_name].append(token.value)
                        # LOGGER.info(f"Partial terms: {', '.join(partial_terms)}")
                ...
                if not is_parse_successful and production_name == "":
                    # Recover production name in case parse fails
    ...
    def dump_bnf_grammar_elements(self):
        ...
        with open(bnf_elements_json_path, mode="w", encoding="utf-8") as bnf_elements_json_file:
            json.dump([x.get_as_dict() for x in self.elements], bnf_elements_json_file, default=lambda obj: asdict(obj), indent=2, sort_keys=True, ensure_ascii=False)
    ...
    def export_html(self):
        ...
        # Report unresolved non-terminals
        if self.unresolved_non_terminals_dict:
            LOGGER.warning(f"Grammar {self.bnf_path} contains {len(self.unresolved_non_terminals_dict)} unresolved non-terminal(s) ...")
            for non_terminal in sorted(self.unresolved_non_terminals_dict.keys()):
                lines_string = "    " + "\n    ".join(self.unresolved_non_terminals_dict[non_terminal])
                LOGGER.error(f"Cannot resolve {non_terminal} in\n{lines_string}")
...
def remove_ws_surrounding_list_elements(html_snippet: str) -> str:
    """Return html string with all whitespace, including newlines, surrounding ol, ul and li elements removed"""
...
def render_nested_lists(html_snippet: str, mode: RenderMode, apply_line_comment: bool) -> str:
    ...
    The html_snippet must be pretreated with functions
    - normalize_whitespace
    - remove_ws_surrounding_list_elements
    """
    indent = 2 * " "
    bullet = chr(0x2022)
    ...
    level = 0
    first_char_after_list_end = False
    ...
    while pos < len(html_snippet):
        if html_snippet[pos:pos + 4] == "<ol>":
            ...
            if mode == RenderMode.HTML:
                ...
                pass
            first_char_after_list_end = True
            stack.pop()
            level -= 1
            pos += 5
            ...
            elif mode == RenderMode.TXT and first_char_after_list_end and len(stack) > 0:
                comment_mark = f"{LINE_COMMENT_MARKER} " if apply_line_comment else ""
                buffer.append(f"{comment_mark}{stack[-1].level * indent}")
            first_char_after_list_end = False
...
def main() -> None:
    ...
    log_file_name = "bnf_grammar_processor.log"
    ...
    parser = argparse.ArgumentParser(
        prog="bnf_grammar_processor",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        allow_abbrev=False,
        description="Extract or parse textual and/or graphical grammars from given KerML or SysML specifications and generate plain text and html BNF grammar files.",
        epilog="""\
...
""")
    parser.add_argument("source_data", metavar="SOURCE_DATA", type=str, help="JSON file defining source data - see examples under the tests directory")
    ...
    # Construct the GrammarProcessor and execute
        ...
        for source in source_list:
            ...
            if syntax_kind in ("kerml-library", "sysml-library"):
                ...
                if file_name.endswith("-marked_up.kebnf") or file_name.endswith("-marked_up.kgbnf"):
                    # Parse the given -marked_up grammar files
                    # and run the backend processors
                    ...
                    # Extract the requested BNF grammars from each of the given input file / clause combinations
                    # and run the backend processors
