...
package org.omg.kerml.xtext.tests;
...
/**
 * Standalone (non-OSGi) initialization of the KerML Xtext language, shared by all tests in
 * this bundle. The setup is performed once per JVM, on first use.
 */
public final class KerMLTestSetup {

	private static Injector injector;

	private KerMLTestSetup() {
	}

	/**
	 * Returns the KerML injector, registering the language (file extension {@code .kerml})
	 * with EMF on first call.
	 */
	public static synchronized Injector getInjector() {
		if (injector == null) {
			...
			injector = new KerMLStandaloneSetup().createInjectorAndDoEMFRegistration();
		}
		return injector;
	}
