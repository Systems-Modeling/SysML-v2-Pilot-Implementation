    ...
    CodeMirror.defineMode("sysml", function (config) {
        return CodeMirror.getMode(config, {
            ...
            keywords: words("about abstract accept action activity alias all allInstances analysis any as assert assoc assume attribute bind block by " +
                ...
                "private protected public redefines ref rendering rep require requirement return satisfy send snapshot specializes state stream subsets succession " + 
                "then timeslice to transition type value variant variation verification view viewpoint"
                ),
