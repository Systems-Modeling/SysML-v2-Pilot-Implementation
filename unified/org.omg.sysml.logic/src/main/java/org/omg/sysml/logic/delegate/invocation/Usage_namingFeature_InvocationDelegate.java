...
/**
 * Generated invocation delegate for {@code Usage::namingFeature}.
 */
public class Usage_namingFeature_InvocationDelegate extends AbstractInvocationDelegate {
    ...
    public Object dynamicInvoke(InternalEObject target, EList<?> arguments) throws InvocationTargetException {
         ...
         */
        Feature result = null;
        if (target instanceof Usage self) {
            if (!(self.getOwningMembership() instanceof VariantMembership)) {
                if (!((Feature) self).getOwnedRedefinition().isEmpty()) {
                    result = ((Feature) self).getOwnedRedefinition().getFirst().getRedefinedFeature();
                }
            } else {
