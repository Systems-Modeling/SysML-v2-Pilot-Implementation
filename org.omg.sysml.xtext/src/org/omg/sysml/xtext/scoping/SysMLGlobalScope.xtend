...
class SysMLGlobalScope extends AbstractScope {
	...
	static def createScope (IScope outer, Resource resource, Predicate<IEObjectDescription> filter, Predicate<IEObjectDescription> rootFilter, EClass type, SysMLScopeProvider scopeProvider) {
		return new SysMLGlobalScope(outer, resource, filter, rootFilter, type, scopeProvider);
	}

	new(IScope outer, Resource resource, Predicate<IEObjectDescription> filter, Predicate<IEObjectDescription> rootFilter, EClass type, SysMLScopeProvider scopeProvider) {
	...
	def IScope alfScope(Package pack) {
		new SysMLScope(IScope.NULLSCOPE, pack, this.referenceType, this.scopeProvider, false, null)
	}
