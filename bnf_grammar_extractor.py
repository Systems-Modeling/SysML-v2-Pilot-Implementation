...
def extract_ebnf(html_spec_path: str, ebnf_spec_path: str, textual_notation_clause_number: str, format: str = "text", token: str = ""):
    ...
    if format == "html":
        ebnf_spec.write("<html>\n<head><style>img {margin-left:35pt;}</style></head>\n<body>\n<pre>")
    ...
    for tag in soup.find_all():
        ...
        if tag.name == "h1":
            ...
            for subtag in tag:
                ...
                elif subtag.name == "img":
                    alt_attr = subtag.attrs.get("alt")
                    height_attr = subtag.attrs.get("height")
                    width_attr = subtag.attrs.get("width")
                    src_attr = subtag.attrs.get("src")
                    if src_attr:
                        src_attr_parts = src_attr.split("?")
                        src_attr_location = src_attr_parts[0]
                        img_token = src_attr_parts[1]
                        img_tokens.add(img_token)
                        src_attr = f"{src_attr_location}?{token}"
                    else:
                        src_attr_location = "NONE"
                    if format == "html":
                        ebnf_spec.write(f'      </pre><img alt="{alt_attr}" height="{height_attr}" width="{width_attr}" src="{src_attr}"/><pre>\n')
                    else:
                        ebnf_spec.write(f'      <img alt="{alt_attr}" height="{height_attr}" width="{width_attr}" src="{src_attr}"/>\n')
                else:
    ...
    ebnf_spec.write("\n")
    ebnf_spec.write(line_comment("Start of GBNF Grammar Checks"))
    img_tokens_block = []
    img_tokens_block.append("img_tokens:")
    for img_token in sorted(img_tokens):
        img_tokens_block.append(img_token)
    ebnf_spec.write(block_comment(img_tokens_block))
    ebnf_spec.write(line_comment("End of GBNF Grammar Checks"))
...
if __name__ == "__main__":
    ...
    ebnf_spec_path = "data/sysml_v2_gbnf.txt"
    # textual_notation_clause_number = "8.2.2"
    textual_notation_clause_number = "8.2.3"

    extract_ebnf(html_spec_path, ebnf_spec_path, textual_notation_clause_number, "html",
                 # token="eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJocGRla29uaW5nIiwiY3JlYXRlZCI6MTc0MzY2ODM5MTcxMiwiaWQiOiJocGRla29uaW5nIiwiZW5hYmxlZCI6dHJ1ZSwiYXV0aG9yaXRpZXMiOlsic3N0IiwiZXZlcnlvbmUiXSwiZXhwIjoxNzQzNzU0NzkxfQ.TdmY7fM4z7fpU3KGDoT57eKIoWmzhdLgk3WLiG--3pU")
                 token="eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJocGRla29uaW5nIiwiY3JlYXRlZCI6MTc0Mzc2NjI0OTYwOCwiaWQiOiJocGRla29uaW5nIiwiZW5hYmxlZCI6dHJ1ZSwiYXV0aG9yaXRpZXMiOlsic3N0IiwiZXZlcnlvbmUiXSwiZXhwIjoxNzQzODUyNjQ5fQ.rPVryU6OtojO3Wa_hfTd39FvY63G4_9YhEYj2uw9sFw")
