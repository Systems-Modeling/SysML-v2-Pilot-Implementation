...
/**
 * Generated invocation delegate for {@code Type::allSupertypes}.
 */
public class Type_allSupertypes_InvocationDelegate extends AbstractInvocationDelegate {
    ...
    public Object dynamicInvoke(InternalEObject target, EList<?> arguments) throws InvocationTargetException {
        ...
        if (target instanceof Type self) {
            var collected = new LinkedHashSet<Type>();
            var pending = new ArrayDeque<Type>();
            collected.add(self);
            pending.add(self);
            while (!pending.isEmpty()) {
                var current = pending.removeFirst();
                for (var supertype : current.supertypes(false)) {
                    if (supertype != null && collected.add(supertype)) {
